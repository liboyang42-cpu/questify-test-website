...
function scriptCaseCard(item, index) {
  return `<article class="script-case-card" data-script-open="${index}">
    ...
    <div class="script-case-card-content">
      ...
      <button class="script-case-button" type="button">打开剧本</button>
    </div>
  </article>`;
}

function scriptReaderOverlay(item) {
  return `<div class="script-reader" data-script-reader aria-hidden="true">
    <div class="script-reader-shell">
      <button class="script-reader-close" type="button" data-script-close aria-label="关闭剧本">×</button>
      <div class="script-reader-progress" data-script-progress>${item.chapters.map((chapter, index) => `<button type="button" data-script-jump="${index}"><span>${chapter.num}</span>${chapter.title}</button>`).join('')}</div>
      <div class="script-scenes" data-script-scenes>
        <section class="script-scene script-scene-cover" data-script-scene style="--scene-bg:url('${item.chapters[0].image}')">
          <video class="script-cover-video" muted autoplay loop playsinline preload="metadata" src="${item.video}"></video>
...
function setupScriptReader(root) {
  ...
  const setActive = (index) => {
    jumpButtons.forEach((button, i) => button.classList.toggle('active', i === index - 1));
  };
