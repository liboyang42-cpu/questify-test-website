...
export function mountLanding(root, scheduler) {
  root.innerHTML = `
    ...
    <section class="landing-scene vex-scene">
      ...
      <nav class="vex-nav cinematic-nav">
        <div class="liquid-glass vex-nav-bar">
          <strong>VEX PLUGIN LAB</strong>
          <div class="vex-nav-links">
            <a>Index</a><a>Modules</a><a>Field</a><a>Notes</a>
          </div>
          <button class="cinematic-button">Open Lab</button>
        </div>
      </nav>
      <div class="vex-content">
        <div class="vex-shell">
          <aside class="vex-sidebar">
            <div class="vex-sidebar__head">
              <p class="label">Plugin Index</p>
              <h1 data-animated-heading></h1>
              <p class="vex-sub">Questify’s city systems, tuned as a compact experiment library instead of a normal landing page.</p>
            </div>
            <div class="vex-filters">${vexFilterPills()}</div>
            <div class="vex-plugin-list">
              ${vexPlugins.map(vexPluginItem).join('')}
            </div>
          </aside>
          <div class="vex-field">
            <div class="vex-field__frame liquid-glass-strong">
              <div class="vex-field__scanline"></div>
              <div class="vex-field__orbit vex-field__orbit--one"></div>
              <div class="vex-field__orbit vex-field__orbit--two"></div>
              <div class="vex-field__core">
                <span>VEX SYSTEM</span>
                <strong>live plugin lattice</strong>
                <p>Black canvas. Liquid overlays. Modular city systems mapped to a compact experimental field.</p>
              </div>
            </div>
            ${vexPreviewCard(vexPlugins[0])}
          </div>
          <aside class="vex-brief">
            <article class="liquid-glass-strong vex-note">
              <p class="label">[SYSTEM WARNING]</p>
              <h3>The lab should feel internal, precise, and a little dangerous.</h3>
              <p>Keep the city playable. Keep the chrome quiet. Let every module read like a tool, not a campaign card.</p>
            </article>
            <article class="liquid-glass-strong vex-note">
              <p class="label">[WHY]</p>
              <h3>Plugins give Questify a sharper R&D posture.</h3>
              <p>Each system can stand alone, then compose into routes, rewards, events, and social loops without collapsing the identity.</p>
            </article>
            <article class="liquid-glass-strong vex-note">
              <p class="label">[CURRENT MODULES]</p>
              <p class="vex-note__list">${vexPlugins.slice(0, 4).map((item) => item.title).join(' · ')}</p>
            </article>
            <article class="liquid-glass-strong vex-note">
              <p class="label">[ABOUT]</p>
              <p>Questify / 城瘾 turns streets, merchants, cameras, and clues into playable infrastructure for the city.</p>
            </article>
          </aside>
        </div>
      </div>
  ...
  setupVexHeading(root.querySelector('[data-animated-heading]'), 'VEX Plugin Library\nfor city experiments.');
...
function vexPluginItem(item, index) {
  return `<article class="vex-plugin-item ${index === 0 ? 'is-active' : ''}" data-vex-plugin="${item.tag}">
    <div class="vex-plugin-item__meta">
      <span>${item.code}</span>
      <span>${item.tag}</span>
    </div>
    <h3>${item.title}</h3>
    <p>${item.summary}</p>
    <div class="vex-plugin-item__signals">
...
function vexFilterPills() {
  return vexFilters.map((tag) => `<span class="liquid-glass vex-filter-pill">${tag}</span>`).join('');
}

function vexPreviewCard(item) {
  return `<article class="liquid-glass-strong vex-preview-card">
    <div class="vex-preview-card__top">
      <span>${item.code}</span>
      <span>${item.signal}</span>
    </div>
    <h3>${item.title}</h3>
    <p>${item.detail}</p>
    <div class="vex-preview-grid">
      <div><span>TAG</span><strong>${item.tag}</strong></div>
      <div><span>FOCUS</span><strong>${item.metricLabel}</strong></div>
      <div><span>STATE</span><strong>${item.metricValue}</strong></div>
    </div>
  </article>`;
}
      ...
      <div class="script-scenes" data-script-scenes tabindex="-1">
        <section class="script-scene script-scene-cover" data-script-scene style="--scene-bg:url('${item.chapters[0].image}')">
          ...
          <div class="script-cover-copy">
            ...
            <nav class="script-cover-index" aria-label="${item.title} 完整章节目录">
              ${item.chapters.map((chapter, index) => `<button type="button" data-script-jump="${index}"><span>${chapter.num}</span>${chapter.title}</button>`).join('')}
            </nav>
            <button class="script-start-button" type="button" data-script-start>从序章开始</button>
          </div>
        </section>
...
function setupScriptReader(root) {
  ...
  const progressButtons = [...reader.querySelectorAll('[data-script-progress] [data-script-jump]')];
