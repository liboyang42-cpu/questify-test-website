import { cubeCarouselStyles, cubeCarouselMarkup, bindCubeCarousel, readSavedCubeIndex } from './ip-cube-carousel.js';
  ...
  return `
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .mark { position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 54px; height: 32px; pointer-events: auto; color: #fff; }
      .mark svg { width: 100%; height: 100%; display: block; }
      .topbar-nav { display: flex; gap: 24px; align-items: center; pointer-events: auto; }
...
function renderHome() {
  ...
  bindCubeCarousel(home, {
    projects,
    projectVisual,
    wrapIndex: wrapProjectIndex,
    captureWheel: true,
    initialIndex: readSavedCubeIndex(wrapProjectIndex),
    onEnter(project) {
...
function renderDetail() {
  ...
  root.innerHTML = `
    ...
    <div class="page project-${project.id}">
      ...
      <section class="home home--page" id="ip-cubes">
        ${cubeCarouselMarkup()}
      </section>
  ...
  const cubeHome = document.querySelector('.home--page');
  ...
  window.scrollTo(0, cubeHome.offsetHeight);
  requestAnimationFrame(() => {
    document.documentElement.style.scrollBehavior = '';
  });

  bindCubeCarousel(cubeHome, {
    projects,
    projectVisual,
    wrapIndex: wrapProjectIndex,
    captureWheel: false,
    ...
    onEnter(next) {
      ...
      document.body.classList.remove('loaded');
      setTimeout(() => {
        window.location.href = `./ip.html?project=${next.id}`;
      }, 280);
    }
  });
  ...
  function updateMotion() {
    const origin = cubeHome ? cubeHome.offsetHeight : 0;
    const y = Math.max(0, (window.scrollY || 0) - origin);
